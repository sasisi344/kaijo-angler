...
📌 メモ（2026-07-14）: GSC/GA4など**未来のデータが揃わないと判定できないタスク**（301リダイレクト効果測定・fish-strategyリライト効果測定・月次GSC再分析・TABILMO P1クリック実績・観光導線go/no-go判断）は[[weekly-task]]冒頭の「保留中：将来のGSC/GA4データが必要なタスク一覧」に集約した。今後この種のタスクが増えたら都度そちらに追記する。
