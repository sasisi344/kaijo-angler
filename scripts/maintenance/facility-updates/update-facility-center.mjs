...
async function updateFacilityData() {
  ...
  // 中日本ターゲット: center-japan フォルダ or mie フォルダ
  const allFiles = getAllFiles(CONTENT_DIR).filter(file => {
    return file.endsWith('.md') && (file.includes('center-japan') || file.includes('mie'));
  });

  console.log(`Updating ${allFiles.length} files for Center Japan...`);
