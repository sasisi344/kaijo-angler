...
async function updateFacilityData() {
  ...
  // 東日本ターゲット: east-japan フォルダ
  const allFiles = getAllFiles(CONTENT_DIR).filter(file => {
    return file.endsWith('.md') && file.includes('east-japan');
  });

  console.log(`Updating ${allFiles.length} files for East Japan...`);
